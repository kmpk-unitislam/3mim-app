  ...
  return (
    ...
    >
      <div className="tab-card-content p-8 sm:p-10 landscape:p-5 flex flex-col items-center justify-center h-full">
        {/* Icon */}
        <div className={`w-14 h-14 sm:w-16 sm:h-16 rounded-full bg-teal flex items-center justify-center mb-4 sm:mb-6 landscape:mb-3 transition-all duration-500 ${isActive ? 'scale-110' : ''}`}>
          {icon}
        </div>

        {/* Text Content */}
        <div className="text-center w-full">
          <h3 className="text-xl sm:text-2xl landscape:text-lg md:landscape:text-xl font-display font-bold text-teal mb-1 sm:mb-2 landscape:mb-1">
            {title}
          </h3>
          <p className="text-gold font-medium italic mb-2 sm:mb-4 landscape:mb-2 text-sm sm:text-base landscape:text-xs md:landscape:text-sm">{subtitle}</p>
          <p className="text-gray-600 text-sm landscape:text-xs md:landscape:text-sm leading-relaxed">
            {description}
          </p>
        </div>
      </div>
    ...
    <div id="tiga-mim-container" className="relative w-full bg-[#f0faf7]">
      <div className="relative w-full min-h-screen flex flex-col justify-center max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 landscape:py-8">
        ...
        <div className="w-full text-center">
          <h2 className="text-5xl sm:text-6xl md:text-7xl landscape:text-4xl md:landscape:text-5xl lg:landscape:text-7xl font-display font-bold text-teal mb-2 sm:mb-4">
            3 MIM
          </h2>
          <p className="text-lg sm:text-xl landscape:text-base md:landscape:text-lg lg:landscape:text-xl text-gray-600 font-display">
